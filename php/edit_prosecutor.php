...
$prosecutor_id=mysqli_real_escape_string($con, $_POST['prosecutor_id']);
...
$update_prosecutor= mysqli_query($con, "UPDATE `cj_investigation`.`prosecutor` SET `name` = '$prosecuter_name', `updatedate` = NOW(), `status` = '$status', `pros_id` = '$pros_id' WHERE `prosecutor`.`id` = '$prosecutor_id';");

if ($update_prosecutor) {
    mysqli_commit($con);
    header('Location: ../prosecutor_profile.php?id='.$prosecutor_id.'&backresult=1'); //رقم الحصر مكرر
    exit;
}else{
    header('Location: ../prosecutor_profile.php?id='.$prosecutor_id.'&backresult=0');
    exit;//if ($insert_case){
}
