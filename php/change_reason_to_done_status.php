...
$reason_to_done_id=mysqli_real_escape_string($con, $_GET['reason_to_done_id']);
$status=mysqli_real_escape_string($con, $_GET['status']);

$update_reason_to_done= mysqli_query($con, "UPDATE `cj_investigation`.`reason_to_done` SET `status` = '$status' WHERE `reason_to_done`.`id_reason_to_done` = '$reason_to_done_id';");

if ($update_reason_to_done) {
    mysqli_commit($con);
    header('Location: ../reason_to_done_settings.php?backresult=1'); //رقم الحصر مكرر
    exit;
}else{
    header('Location: ../reason_to_done_settings.php?backresult=0');
    exit;//if ($insert_case){
}
