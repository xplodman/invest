...
$charge_id=mysqli_real_escape_string($con, $_GET['charge_id']);
$status=mysqli_real_escape_string($con, $_GET['status']);

$update_charge= mysqli_query($con, "UPDATE `cj_investigation`.`charges` SET `status` = '$status' WHERE `charges`.`id_charges` = '$charge_id';");

if ($update_charge) {
    mysqli_commit($con);
    header('Location: ../charges_settings.php?backresult=1'); //رقم الحصر مكرر
    exit;
}else{
    header('Location: ../charges_settings.php?backresult=0');
    exit;//if ($insert_case){
}
