        ...
        <div class="container-fluid">
            ...
            <div class="row page-titles">
                <div class="col-md-10 align-self-center">
                    <h3 class="text-themecolor">الإعدادات / الأعضاء</h3>
                </div>
                <div class="">
                    <button class="btn btn-success " type="button" data-toggle="modal" data-target="#add_prosecuter"> إضافة عضو نيابة</button>
                </div>
            </div>
            <!-- ============================================================== -->
            <?php
...
WHERE
  prosecutor.deleted = 0 AND
  pros_has_users.users_id = '$user_id'
ORDER BY 
  prosecutor.status DESC , 
  prosecutor.name";
            ?>
            <!-- ============================================================== -->
            <!-- Start Page Content -->
                            ...
                            <div class="table-responsive">
                                <table id="datatable" class="display table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                    ...
                                    <tr>
                                        ...
                                        <th width="85%">أسم العضو</th>
                                        <th width="85%">حالة العضو</th>
                                        <th width="15%">للتعديل</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    <?php
                                    $result = mysqli_query($con, $prosecutor_query);
                                    while($prosecutor_info = mysqli_fetch_assoc($result)) {
                                        ?>
                                        <tr data-child-value="<?php
                                        ?>">
                                            <td>
                                                <?php echo $prosecutor_info['id']?>
                                            </td>
                                            <td>
                                                <?php echo $prosecutor_info['name']?>
                                            </td>
                                            <td>
                                                <?php
                                                if ($prosecutor_info['status']=='0'){
                                                    ?>
                                                <button type="button" class="btn waves-effect waves-light btn-danger">
غير مفعل
                                                </button>
                                                <?php
                                                }elseif ($prosecutor_info['status']=='1'){
                                                    ?>
                                            ...
                                            <td>
                                                <a href="prosecutor_profile.php?id=<?php echo $prosecutor_info['id'] ?>">
    ...
    $(document).ready(function() {
        $('#datatable').DataTable({
            initComplete: function () {
                this.api().columns(':eq(99),:eq(5)').every( function () {
                    var column = this;
        ...
        $('#datatable tfoot th').not(':eq(99),:eq(5),:eq(6)').each(function() {
            var title = $(this).text();
