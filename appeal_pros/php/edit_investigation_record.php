...
$case_main_ledger=mysqli_real_escape_string($con, $_POST['main_ledger']);
$case_depart=mysqli_real_escape_string($con, $_POST['depart']);
...
$update_case_has_investigation= mysqli_query($con, "UPDATE case_has_investigation SET `investigation_number` = '$investigation_number', `investigation_year` = '$investigation_year', `case_status_idcase_status` = '$case_status', `prosecutor_id` = '$prosecutor', `notes` = '$notes', `receive_date` = '$receive_date', `defendant` = '$defendant', `plaintiff` = '$plaintiff', `updatedate` = NOW() WHERE id_case_has_investigation= $investigation_id;")or die(mysqli_error($con));

if (isset($_POST['initial_action']) && !empty($_POST['initial_action'])) {
    $initial_action = mysqli_real_escape_string($con, $_POST['initial_action']);
    $update_case_has_investigation_initial_action= mysqli_query($con, "UPDATE case_has_investigation SET `initial_action_id` = '$initial_action', `updatedate` = NOW() WHERE id_case_has_investigation= $investigation_id;")or die(mysqli_error($con));

}

if (isset($_POST['final_action']) && !empty($_POST['final_action'])) {
    $final_action = mysqli_real_escape_string($con, $_POST['final_action']);
    $update_case_has_investigation_final_action= mysqli_query($con, "UPDATE case_has_investigation SET `final_action_id` = '$final_action', `updatedate` = NOW() WHERE id_case_has_investigation= $investigation_id;")or die(mysqli_error($con));
